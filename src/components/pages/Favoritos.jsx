...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await axios.get('https://backendauth-production.up.railway.app/api/auth/user', { withCredentials: true });
        setUser(response.data);

        // Obtener las imágenes del usuario
        const userResponse = await axios.get(`https://backendapi-production-f363.up.railway.app/api/user/${response.data._id}/images`);
  ...
  const handleFavoriteClick = async (imageId) => {
    if (user) {
      try {
        ...
        await axios.post(`https://backendapi-production-f363.up.railway.app/api/user/${user._id}/favorite`, {
          imageId: imageId,
        });
  ...
  const handleLogout = async () => {
    try {
      await axios.post('https://backendauth-production.up.railway.app/api/auth/logout');
      setUser(null);
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDeleteImage = async (imageId) => {
    try {
      await axios.delete(`https://backendapi-production-f363.up.railway.app/api/${imageId}`);
