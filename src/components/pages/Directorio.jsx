...
const Directorio = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await axios.get('https://backendauth-production.up.railway.app/api/auth/user');
    ...
    const fetchFavoriteImages = async () => {
      try {
        if (user && user.favorites) {
          const response = await axios.post('https://backendapi-production-f363.up.railway.app/api/favorites', { favoriteIds: user.favorites });
